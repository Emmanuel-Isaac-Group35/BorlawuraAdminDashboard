...
export default function Sidebar({ activeSection, setActiveSection, isOpen, onClose, adminInfo }: SidebarProps) {
  ...
  const getMenuItems = () => {
    ...
    const allItems = [
      ...
      { id: 'push', icon: 'ri-notification-3-line', label: 'Push Alerts', roles: ['admin', 'manager'], color: 'indigo' },
      { id: 'sms', icon: 'ri-chat-voice-line', label: 'SMS', roles: ['admin', 'manager', 'support'], color: 'emerald' },
      { id: 'feedback', icon: 'ri-customer-service-2-line', label: 'Support Desk', roles: ['admin', 'manager', 'support'], color: 'emerald' },
      { id: 'analytics', icon: 'ri-bar-chart-box-line', label: 'Reports', roles: ['admin', 'finance', 'manager'], color: 'orange' },
