...
export default function CMSManagement({ adminInfo }: CMSManagementProps) {
  ...
  const [activeTab, setActiveTab] = useState<'interface' | 'banners' | 'popup' | 'support' | 'links' | 'theme'>('interface');
                           ...
                           <div key={banner.id} className={`p-5 bg-slate-50 dark:bg-black/30 rounded-2xl border transition-all group ${banner.enabled ? 'border-transparent hover:border-violet-500/10' : 'border-rose-500/20 opacity-60'}`}>
                              <div className="flex flex-col lg:flex-row gap-6">
                                 ...
                                 <div className="flex flex-row lg:flex-col justify-end lg:justify-center gap-3">
                                    <button onClick={() => { const b = [...currentApp.banners]; b[idx].enabled = !b[idx].enabled; updateField(`${activeApp}.banners`, b); }} className={`w-10 h-10 lg:w-12 lg:h-12 rounded-xl flex items-center justify-center transition-all shadow-sm ${banner.enabled ? 'bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500 hover:text-white' : 'bg-slate-200 text-slate-400 hover:bg-slate-300'}`}>
                                       <i className={banner.enabled ? 'ri-eye-line text-lg' : 'ri-eye-off-line text-lg'}></i>
                                    </button>
            ...
            <div className="relative w-[320px] h-[650px] bg-slate-900 rounded-[3.5rem] border-[10px] border-slate-900 shadow-2xl overflow-hidden mx-auto transition-transform hover:scale-[1.01] duration-700">
               <div className={`absolute inset-0 flex flex-col ${currentApp.theme.darkMode ? 'bg-[#0f0f12] text-white' : 'bg-white text-slate-900'}`}>
                  ...
                  <div className="flex-1 px-6 mt-6 space-y-5 overflow-hidden">
                     {currentApp.bannersEnabled && currentApp.banners.some((b: any) => b.enabled) ? (
                        <div className="h-40 bg-slate-50 dark:bg-white/5 rounded-3xl overflow-hidden relative shadow-sm">
                           {(() => {
                              const activeBanner = currentApp.banners.find((b: any) => b.enabled);
                              return activeBanner?.image ? (
                                 <div className="w-full h-full relative">
                                    ...
                                    <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent p-5 flex flex-col justify-end">
                                       <span className="text-[7px] font-bold text-white/50 uppercase tracking-widest leading-none mb-1">{activeBanner.subtitle}</span>
                                       <h5 className="text-[10px] font-black text-white uppercase tracking-tight leading-none">{activeBanner.title}</h5>
                                    </div>
                                 </div>
                              ) : (
                        ...
                        <div className="h-40 border-2 border-dashed border-slate-100 dark:border-white/5 rounded-3xl flex flex-col items-center justify-center opacity-20">
                           <i className="ri-slideshow-3-line text-2xl mb-1"></i>
                           <span className="text-[7px] font-black uppercase tracking-widest text-center px-10">
                              {!currentApp.bannersEnabled ? 'Carousel Global Switch: OFF' : 'No Active Banners'}
                           </span>
                        </div>
                     )}
             ...
             <div className="flex flex-col items-center gap-1.5 mt-4">
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.4em]">Device Preview</span>
